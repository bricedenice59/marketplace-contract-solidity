...
const addCourseOwnerToContract = async (
  web3,
  deployedContract,
  account,
  newCourseOwner
  ...
  const method = deployedContract.methods.addCourseOwner(
    newCourseOwner.id,
    newCourseOwner.address,
    newCourseOwner.rewardPercentage
  );
...
const addCourseToContract = async (
  web3,
  deployedContract,
  account,
  newCourse
  ...
  const gas = await web3.eth.getGasPrice();
  const method = deployedContract.methods.addCourse(
    newCourse.id,
    newCourse.title,
    newCourse.price,
    newCourse.courseOwner
  );
  const createTransaction = await web3.eth.accounts.signTransaction(
    {
      from: account.address,
...
const activateCourse = async (
  ...
  const gas = await web3.eth.getGasPrice();
  var method = shouldActivate
    ? deployedContract.methods.activateCourse(course.id)
...
const getCoursePrice = async (deployedContract, account, course) => {
  var res = await deployedContract.methods
    .getCoursePrice(course.id)
    .call({ from: account.address }, function (err, result) {
      if (err) {
        console.log(
          `An error occured when trying to get course price for ${course.id}`,
          err
        );
        return;
      }
...
const getUserBoughtCoursesIds = async (deployedContract, account) => {
  var res = await deployedContract.methods
    .getUserBoughtCoursesIds(account.address)
    .call(function (err, result) {
      if (err) {
        console.log(
          `An error occured when trying to get user purchases; address: ${account.address}`,
          err
        );
        return;
      }
