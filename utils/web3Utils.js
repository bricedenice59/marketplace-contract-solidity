...
const baseContractSolidityFile = path.resolve("contracts", "MarketplaceContract.sol");
const rinkebyDefaultGas = 5000000; // Rinkeby has a lower block limit than mainnet

const CreateWeb3Connection = () => {
    return new Web3(new Web3.providers.WebsocketProvider(process.env.INFURA_WS_NETWORK));
}
...
const getDeployedContract = (web3, deployedContractAddress) => {
    const baseContractFileName = path.basename(baseContractSolidityFile);
    return new web3.eth.Contract(getCompiledContract().contracts[baseContractFileName][baseContractName].abi, deployedContractAddress);
}
